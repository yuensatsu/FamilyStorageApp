...
- リポジトリ内に`SPEC.md`を追加済みです。
- 本TODOでは以下を比較元・比較対象として扱います。
  - `SPEC.md`に定義された「金魚すくい3D」仕様
...
- `SPEC.md`を追加済み。
- 仕様比較の単一参照元として利用できる状態になっている。

**対応内容:**

- 仕様の単一参照元として`SPEC.md`を追加した。
- 以下を明記済み。
  - 対象プラットフォーム: Android
  - シーン構成
  - ゲームルール
...
### P2-06 `[進行中]` `ARCH.md`と`TODO.md`を仕様更新に合わせて保守する
...
- `SPEC.md`を追加し、`TODO.md`と`ARCH.md`の参照関係を更新済み。

**対応内容:**

- 仕様変更時に`SPEC.md`を更新する。
- `SPEC.md`更新後にTODOを再比較する。
- 実装完了に合わせてステータスを更新する。
...
1. Unityプロジェクトとして開ける構成を追加する
2. `GoldfishScene`を作成する
3. 金魚Prefabとポイを作成する
4. Editorで60秒ゲームとして最後まで遊べることを確認する
5. ハイスコア保存とSNS共有を実機確認する
